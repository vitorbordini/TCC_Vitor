...
class TensorboardWriter():
    ...
    def __init__(self, args):
        name_model = args.model + "_" + args.opt + "_" + args.loss+ "_" + str(args.noise_train) + "_" + str(args.noise_val) + "_" + args.noise_type
        self.writer = SummaryWriter(log_dir=args.log_dir + name_model, comment=name_model)
        utils.make_dirs(args.log_dir + name_model+"/")
        ...
        #create file with all inputs
        with open(args.log_dir + name_model+"/"+ "parameters.txt", 'w') as f:
            f.write("Loss:{}\n".format(args.loss))
            f.write("Batch Size:{} \n".format(args.batchSz))
            f.write("Epochs:{} \n".format(args.nEpochs))
            f.write("Model:{} \n".format(args.model))
            ...
            f.write("Optimizer:{} \n".format(args.opt))
            f.write("Samples Train:{} \n".format(args.samples_train))
            f.write("Samples Validation:{} \n".format(args.samples_val))
            f.write("Augmentation:{} \n".format(args.augmentation))
            f.write("Normalization:{} \n".format(args.normalization))
            f.write("Noise train:{} \n".format(args.noise_train))
            f.write("Noise val:{} \n".format(args.noise_val))
    ...
    def display_terminal(self, iter, epoch, mode='train', summary=False):
        ...
        """
        if summary:
            info_print = "\nSummary {} Epoch {}:  Loss:{:.4f} \t DSC:{:.4f}  ".format(mode, int(epoch),
                                                                                         self.data[mode]['loss'] /
            ...
            info_print = "\nEpoch: {} Loss:{:.4f} \t DSC:{:.4f}".format(int(iter), self.data[mode]['loss'] /
                                                                            self.data[mode]['count'],
                                                                            self.data[mode]['dsc'] /
                                                                            self.data[mode]['count'])
    ...
    def imshow(self,output,label,mode,layer=50):
        print("Inicio das imagens")
        output_img = output[0,0,:,:,layer].cpu()
        label_img = label[0,:,:,layer].cpu()
        
        
        self.writer.add_image("{}/".format(mode),output_img.unsqueeze(0))
        self.writer.add_image("{}/".format(mode),label_img.unsqueeze(0))
        
        plt.imshow(output_img.detach().numpy())
        plt.imshow(label_img.detach().numpy())
